...
def insert_scan_chain_new(fname,verbose=False,presult=True,\
    ...
    #file -->> m_list
    info=np.parse_to_parse_list(input_file)
    m_list=[]
    m_list.append(info[0])
    m_list=m_list+info[3]
    port_decl_list  =info[1]
    signal_decl_list=info[2]
    assign_stm_list=None
    if len(info)==5:
    ...
    for eachPrimitive in m_list[1:]:
        if eachPrimitive.m_type=='LUT' and (eachPrimitive.name in lut_out2_FD_dict.keys()):
            ...
            if input_num==1:
                assert  (init_legal.groups()[1]=='2' and init_legal.groups=="1"),\
                "Error:find LUT1 .INIT !=2'h1 %s" % eachPrimitive.name
                NEW_INIT="8'hC5"
            else:
            ...
            scan_out_dict[counter]=all_fd_dict[lut_out2_FD_dict[eachPrimitive.name][1]]['Q'].string
            cnt_edited_lut+=1
        elif (eachPrimitive.m_type=='FD'):
            if (eachPrimitive.name not in FD_din_lut_list):
                ...
                scan_out_dict[counter]='scan_out'+str(counter)
                fd_replace_cnt+=1
            else:
                pass
    #--------------------------------------------------------------------------
...
if __name__=='__main__':
    if len(sys.argv)==1:
        ...
        insert_scan_chain_new(fname,K=k,verbose=True)
