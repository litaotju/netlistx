...
def get_edge_vertex(m_list,verbose=True):
    ...
    if verbose:
        ...
        print '-----------------------------------------------'
        print 'Info:edge number is:'+str(len(edge_set2))        
        print 'Info:edge_set   ARE:'
        for eachEdge in edge_set2:
            print str(eachEdge)+"::::["+vertex_set_dict[eachEdge[0]]\
                    +"-->>"+vertex_set_dict[eachEdge[1]]+']'
        print '-----------------------------------------------'
                ...
                if eachVertex==eachEdge[0]:
                    ...
                    for eachEdge2 in edge_set_tmp:
                        if eachVertex==eachEdge2[1]:
                            ...
                            if [s_node,d_node] not in edge_set_cpy:
                                edge_set_cpy.append([s_node,d_node])
            vertex_set_cpy.remove(eachVertex) 
            
#    for eachEdge in edge_set_cpy:
#        for eachNode in eachEdge:
#            if eachNode not in vertex_set_cpy:
#                vertex_set_cpy.append(eachNode)
    print 'Info: s_vertex:-----------------------------------------------------'
    for eachVertex in vertex_set_cpy:
        print eachVertex+'::::'+vertex_set_dict[eachVertex]
    print 'Info: s_edge  :-----------------------------------------------------'
